...
class MilitarySymbol:
    ...
    def get_svg(self, style='light', pixel_padding=-1, use_variants=False):
        ...
        if not self.status.makes_frame_dashed and (self.status is not None and (not use_variants or (use_variants and len(self.status.variants) > 0 and self.status.variants[0] != 'nn'))):
